 ...
 */
package de.cismet.cids.server.rest.data.unused;
...
import de.cismet.cids.server.rest.data.configkeys.CidsClassConfigurationFlagKey;
import de.cismet.cids.server.rest.data.configkeys.CidsClassConfigurationKey;
    ...
    public void serialize(final CidsClass c, final JsonGenerator jg, final SerializerProvider provider)
        ...
        if (attributesSet.size() > 0) {
            ...
            for (final Map.Entry<String, CidsAttribute> attr : attributesSet) {
                jg.writeObjectField(attr.getValue().getAttributeKey(), attr.getValue());
            }
            jg.writeEndObject();
        }
        jg.writeEndObject();
